...
class MoviesAdapter(val onClick: (view: View, index: Int) -> Unit) :
    PagingDataAdapter<MoviePagingDomain, MoviesAdapter.ViewHolder>(diffCallback) {

    inner class ViewHolder(private val binding: ItemMovieTvshowBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(data: MoviePagingDomain) {
            ...
            if (data.title.contains("Jiu Jitsu")) {
                println(data)
            }
    ...
    companion object {
        val diffCallback = object : DiffUtil.ItemCallback<MoviePagingDomain>() {
            override fun areItemsTheSame(
                oldItem: MoviePagingDomain,
                newItem: MoviePagingDomain
            ): Boolean =
